...
referencing the provider's key variable — never the resolved secret. A keyless
local provider SHALL get a dummy literal placeholder instead, because Pi hides
a provider's models from `/model` until some auth is configured; local servers
ignore the value.
