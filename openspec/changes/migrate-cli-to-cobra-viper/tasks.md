...
- [ ] 1.1 Add `github.com/spf13/cobra` and `github.com/spf13/viper` to
  `go.mod` and run `go mod tidy` (covers the transitive pflag/afero/cast/
  mapstructure/fsnotify)
- [ ] 1.2 Create the Cobra root command and the subcommand tree in
...
- [ ] 1.3 Give every command an empty `RunE` that delegates to the existing
...
- [ ] 1.4 Verify the skeleton: `go build ./...`, `go vet ./...`, gofmt clean,
  and `go test ./... -cover` green with total coverage >= 80%

## 2. Flag migration: selection and catalogue commands

- [ ] 2.1 Move `add` and `remove` flags off `parseSelection`'s `FlagSet` and
...
- [ ] 2.4 Migrate `serve` (`--dry-run/-n`, `--api/-a`, `--api-addr`,
  `--api-token-file`, `--api-token`, `--log-level`, path) and `daemon`
  (`--api-addr`, `--loopback/-l`, `--api-token-file`, `--api-token`,
  `--log-level`)
- [ ] 2.5 Make `fleet` a parent whose child flag sets live on the children
  ...
  spelling working
- [ ] 2.6 Add focused pflag parse regression tests for the tricky forms
  (shorthand clusters, `--flag=value` attached, unknown-flag errors, bool
  shorthands) for at least: `add`, `apply`, `serve`, `fleet metrics`, and
  `remote start`
- [ ] 2.7 Verify: `go vet ./...`, gofmt, `go test ./... -cover` green,
  coverage >= 80%

## 3. `harness` command parsing (DisableFlagParsing)

- [ ] 3.1 Set `DisableFlagParsing: true` on the `harness` command and port its
...
- [ ] 3.2 Preserve the existing rules exactly: a leading positional naming an
...
- [ ] 3.3 Add parse tests for the harness corner cases: bare `-O` applies
...
- [ ] 3.4 Verify: run the existing harness launch/forwarding tests, full
  suite green, coverage >= 80%

## 4. Viper binding (CLI-layer OUTFIT_* reads)

- [ ] 4.1 Construct the single Viper instance in the CLI layer
...
- [ ] 4.2 Migrate `OUTFIT_ALIAS` reads (the `readOutfit` empty-path branch
...
- [ ] 4.3 Migrate the `OUTFIT_REMOTE_*` config lookups: pass a Viper-backed
...
- [ ] 4.4 Leave the D3 "Unchanged" rows untouched and confirm by review:
...
- [ ] 4.5 Add per-variable precedence tests: flag beats env, env beats
...
- [ ] 4.6 Verify: full suite green, coverage >= 80%, no `os.Getenv`
  of a CLI-layer `OUTFIT_*` var remains outside the D3 "Unchanged" list

## 5. Completion on Cobra's engine

- [ ] 5.1 Implement the `completion` command: positional argument validated
...
- [ ] 5.2 Port the dynamic-candidate helpers from `complete.go` into
...
- [ ] 5.3 Register `ValidArgsFunction`s: alias-slot (`apply`, `unapply`,
...
- [ ] 5.4 Register flag completion functions: `--provider/-p` → catalogue
...
- [ ] 5.5 Wire the `--outfit/-O` completion (aliases + files) and add
  end-to-end `__complete` cases for both attached forms
  (`--outfit=<partial>` and the `--outfit` `=` `` split); if Cobra does not
  invoke the function for the `NoOptDefVal` value, intercept the
  `--outfit=`/`-O=`-prefixed token in the command's `ValidArgsFunction` and
  verify the same output
- [ ] 5.6 Enforce the silence guarantee: every completion function returns
...
- [ ] 5.7 Port the `complete_test.go` scenario assertions through Cobra's
...
- [ ] 5.8 Replace `TestCompletionCoversDispatch`: (a) `__complete ""` lists
...
- [ ] 5.9 Delete the old engine: the `commands` table, `candidateKind`,
...
- [ ] 5.10 Verify: full suite green, coverage >= 80%, and a manual smoke on a
...
- [ ] 6.1 Set the root's `Version` to the build version, keep the `version`
  subcommand, and make `-v` a shorthand for `--version`
- [ ] 6.2 Delete the hand-written `usage()` and update any tests that
  asserted on its exact text to the Cobra-rendered help
- [ ] 6.3 Update the `main` package doc comment (the `// Usage:` block) to
  the Cobra tree and remove now-stale flag-package references from
  `cmd/outfit`
- [ ] 6.4 Verify: `outfit` (no args) prints help and exits 0, `outfit
...
- [ ] 7.1 Update AGENTS.md: the `cmd/outfit` layout section (Cobra tree,
