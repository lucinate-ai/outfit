...
- [ ] 1.1 Add `func Dir() (string, error)` to `internal/config`:
...
- [ ] 3.2 `start/index.ts`: move the engine-log path (CloudWatch agent config)
      to `/var/lib/outfit/daemon/engine.log`
- [ ] 3.3 `image-stack.ts`: point the baked logrotate config at
      `/var/lib/outfit/daemon/engine.log`; bump the runner recipe versions so
      the change re-bakes
- [ ] 3.4 Update `remote/` tests (start user-data asserts the pinned
      `OUTFIT_CONFIG_DIR` and the `/var/lib/outfit` deploy-config/log paths;
      stack test asserts the logrotate path)

## 4. Verification and docs

- [ ] 4.1 `go test ./... -cover` >= 80%, `gofmt` and `go vet` clean;
      `remote/` `pnpm test` green
- [ ] 4.2 Document `OUTFIT_CONFIG_DIR` (README and AGENTS.md — the config
      resolution note), and the `/var/lib/outfit` instance layout in
      `remote/` docs
- [ ] 4.3 `openspec validate config-dir-override --strict` passes
