...
- `cmd/outfit/main.go` — CLI: command dispatch (`add`/`remove`/`list`/`apply`/`export`), flag parsing (`parseSelection` registers both long and short flags against the same vars), and user-facing output. `add` and `apply` share `applySelection`, the core that writes one provider selection. Tests: `main_test.go` (dispatch/add/remove/list), `apply_test.go` (`apply`/`export` end-to-end).
